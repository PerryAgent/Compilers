...
(* The registers of the mips machine *)
    datatype reg    = Imm of int            (* Immediate values are integers *)
                    | Zero 
                    |  At 
                    |  V0 
                    |  V1 
...
(* Function to return register in required string format  *)
    fun printreg (Imm (i)) = Int.toString i
        | printreg Zero = "$zero"
        | printreg At   = "$at"
        | printreg V0   = "$v0"
        | printreg V1   = "$v1"
...
(* Function to convert label to String *)
    fun printlabel (UserDefined s ) = s 
    |   printlabel (TempLabel i)    = "Label_No_" ^ Int.toString i 
...
(* Print the instructions when the labels are strings and registers are actual MIPS registers *)
    fun prInst (Abs(r1,r2))            = "abs "^printreg(r1)^", "^printreg(r2)
        ...
        |    prInst (Div2(r1,r2,r3))   = "div "^printreg(r1)^", "^printreg(r2)^", "^printreg(r3)
        |    prInst (Divu2(r1,r2,r3))  = "divu "^printreg(r1)^", "^printreg(r2)^", "^printreg(r3)
        ...
        |    prInst (Nor(r1,r2,r3))    = "nor "^printreg(r1)^", "^printreg(r2)^", "^printreg(r3)
        ...
        |    prInst (Bgtu(r1, r2, l1)) 		= "bgtu " ^ printreg(r1) ^ ", " ^ printreg(r2) ^ ", " ^ printlabel(l1)
